...
from comet_config import COMET_ML_API
color2num = dict(
    gray=30,
    red=31,
    green=32,
...
class Logger:
    ...
    def __init__(self, output_dir=None, output_fname='progress.txt', exp_name=None,
        ...
        if use_comet: self.comet_args = comet_args
    ...
    def add_tag(self, tag):
        if self.use_comet: self.experiment.add_tag(tag)
    ...
    def log_tabular(self, key, val):
        ...
        if self.use_comet: self.experiment.log_metric(key, val)
    ...
    def save_config(self, config):
        ...
        # Setup cloud logger
        if self.use_comet: self.setup_comet(config)
        
    def setup_comet(self, config):
        self.use_comet = True if COMET_ML_API and self.use_comet else False
        if not self.use_comet: return
        self.experiment = Experiment(
            api_key=COMET_ML_API,
            project_name=self.comet_args['project_name'],
...
class EpochLogger(Logger):
    ...
    def upload_file(self, filepath):
        if not self.experiment: print('comet.ml is not setup'); return
        if os.path.exists(filepath):
            self.experiment.log_asset(filepath)
        else:
            print(f"The following file doesn't exist: {filepath}")

    def wrapup(self):
        if not self.experiment: return
        for file in os.listdir(self.output_dir):
            self.experiment.log_asset(self.output_dir + '/' + file)