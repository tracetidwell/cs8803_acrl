...
class car_problem():
    def __init__(self, map, goal, heuristic_only=False, delay=20, alpha=5):
        ...
        k = np.array([[1,-.3],[-.3,-.3]])
        mapc1 = ndimage.filters.convolve(map, k)

        k = np.array([[-.3,1],[-.3,-.3]])
        mapc2 = ndimage.filters.convolve(map, k)

        k = np.array([[-.3,-.3],[1,-.3]])
        mapc3 = ndimage.filters.convolve(map, k)

        k = np.array([[-.3,-.3],[-.3,1]])
        mapc4 = ndimage.filters.convolve(map, k)

        #print(np.max(mapc4))
        #print(np.unravel_index(np.argmax(np.reshape(mapc4, (50,50))), (50,50)))

        mappy = np.maximum(np.maximum(mapc1, mapc4), np.maximum(mapc2, mapc3)) #, np.maximum # + mapc3 + mapc4

        mappy[mappy < .9] = 0
        self.corners = np.argwhere(mappy > 0)
    ...
    def rrt(self, position):
        ...
        for x in range(0,100000):
            ...
            rando = np.random.sample()
            if ((rando > .15) or (c < 10)):
                ss = sample()
                while self.check_collision(ss):
                    ss = sample()
            elif rando > .05:
                samp = np.random.choice(len(self.corners), 1)
                ss = self.corners[samp[0]]
                print(ss)
            else:
                ss = self.goal
            ...
            if self.viz[int(ss[0]), int(ss[1])] != .6:
                ...
                self.region_viz(self.delay*2)
                self.viz[int(ss[0]), int(ss[1])] = 1

            i = 0
            curr_nearest = nearestk[i]
            rad = 30
            rad = max(nearestk[0][0] * 2, 10)
            #print(rad)

            while i < len(nearestk) and nearestk[i][0] < rad:
                ...
                if self.check_straight_line_possible(nearest, ss):
                    ...
                    while ((best != None) and loops < loop_limit):
                        ...
                        if self.isgoal(best[1]):
                            ...
                            for b in trajectory_list[-1:]:
                                vertices.add(b)
                                if self.viz[int(b[0]), int(b[1])] != .2:
                                    self.viz[int(b[0]), int(b[1])] = .2
